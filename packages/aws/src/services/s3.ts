...
const setFile = async ({bucket, key, metadata = undefined}, content) =>
    awss3.putObject({Bucket: bucket, Key: key, Body: content, ...(metadata ? {Metadata: metadata}: {})}).promise()
;
...
const toJsonFile = async (bucket, key, data, metadata = undefined) => setFileContent({bucket, key, metadata}, JSON.stringify(data));
