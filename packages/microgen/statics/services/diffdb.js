...
const marshallValueAndCheckChanged = (v, p) => {
    ...
    switch (true) {
        ...
        case 'undefined' === t: return [{n: true}, p.n !== true];
...
const getDb = ({name}) => {
    const fetchPaths = async (pr, pa = undefined, throwErrorIfNone = true) => {
    ...
    const applyUpsertChange = async (change, ctx, opts = {}) => {
        ...
        const r = await dynamodb.upsert(name, changeKey(change, ctx), changeData(change, ctx), {values: 'UPDATED_OLD'});
    ...
    const applyChange = async (change, ctx) => {
        try {
            ...
            switch (change.o) {
                ...
                case 'M': return await applyMoveChange(change, ctx);
                case 'S': return await applySkipChange(change, ctx);
    ...
    const applyChangeSet = async query => {
        ...
        const o = Object.keys(c.reduce((acc, cc) => {acc[cc.o] = true; return acc;}, {}));
        o.sort();
