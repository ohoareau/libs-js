...
const dynamodbMock = require('../../services/aws/dynamodb');
const {applyChangeSet, merge, get, getPath} = require('../../services/diffdb').getDb({name: 'table_name'});
...
describe('applyChangeSet', () => {
    ...
    [
        ...
        ([name, query, expected, mockCalls = {}]) => it(name, async () => {
            mockCalls.query && mockCalls.query.forEach(([, exp]) => dynamodbMock.query.mockResolvedValueOnce(exp));
            mockCalls.upsert && mockCalls.upsert.forEach(([, exp]) => (exp instanceof Error) ? dynamodbMock.upsert.mockImplementationOnce(() => { throw exp; }) : dynamodbMock.upsert.mockResolvedValueOnce(exp));
            mockCalls.delete && mockCalls.delete.forEach(([, exp]) => dynamodbMock.delete.mockResolvedValueOnce(exp));
...
describe('get', () => {
    ...
    it('unknown project throw error', async () => {
        dynamodbMock.queryIndex.mockResolvedValue({items: [], count: 0, scannedCount: 0});
    ...
    it('existing project return json-stringified specifications', async () => {
        dynamodbMock.query.mockResolvedValue({items: [
...
describe('getPath', () => {
    ...
    it('existing project and items for sub list path return specifications', async () => {
        dynamodbMock.query.mockResolvedValueOnce({items: [
            {pr: 'p1', pa: 'd.xxx1', _idx: 0, e: 'f'},
            {pr: 'p1', pa: 'd.xxx3', _idx: 2, e: 'g', kk: ['one', 'two', 'three']},
            {pr: 'p1', pa: 'd.xxx2', _idx: 1, e: 'h'},
