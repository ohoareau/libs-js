...
export default {
    delete: async (table, key, returns: any = undefined) => mutateWriteResult(await db.delete(buildParams(table, key, returns)).promise()),
    upsert: async (table, key, data, returns: any = undefined) => mutateWriteResult(await db.update(buildUpdateParams(table, key, data, returns)).promise()),
