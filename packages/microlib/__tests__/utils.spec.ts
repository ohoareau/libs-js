...
describe('createOperationHelpers', () => {
    ...
    it('call specified hook type in a loop with non object items (async) and return array of computed values', async () => {
        ...
        expect(testMock).toHaveBeenNthCalledWith(1, {o: 'x_y_z', model: {name: 'xx'}, dir: expect.any(String), x: 'a'});
        expect(testMock).toHaveBeenNthCalledWith(2, {o: 'x_y_z', model: {name: 'xx'}, dir: expect.any(String), x: 'b'});
        expect(testMock).toHaveBeenNthCalledWith(3, {o: 'x_y_z', model: {name: 'xx'}, dir: expect.any(String), x: 'c'});
    });
    it('call specified hook type in a loop with object items (async) and return array of computed values', async () => {
        ...
        expect(testMock).toHaveBeenNthCalledWith(1, {o: 'x_y_z', model: {name: 'xx'}, dir: expect.any(String), x: 'abc', y: 'a-b-c'});
        expect(testMock).toHaveBeenNthCalledWith(2, {o: 'x_y_z', model: {name: 'xx'}, dir: expect.any(String), x: 'def', y: 'd-e-f'});
        expect(testMock).toHaveBeenNthCalledWith(3, {o: 'x_y_z', model: {name: 'xx'}, dir: expect.any(String), x: 'ghi', y: 'g-h-i'});
    });
    it('call specified hook type in a loop with object items (async) and return array of computed values (recursive replaced values)', async () => {
        ...
        expect(testMock).toHaveBeenNthCalledWith(1, {o: 'x_y_z', model: {name: 'xx'}, dir: expect.any(String), x: {y: {t: 'abc'}}, z: 'hello'});
        expect(testMock).toHaveBeenNthCalledWith(2, {o: 'x_y_z', model: {name: 'xx'}, dir: expect.any(String), x: {y: {t: 'def'}}, z: 'hello'});
        expect(testMock).toHaveBeenNthCalledWith(3, {o: 'x_y_z', model: {name: 'xx'}, dir: expect.any(String), x: {y: {t: 'ghi'}}, z: 'hello'});
    });
});
